...
func (vm *VirtualMachineSpec) Start() error {
	...
	if vm.gcListener != nil {
		...
		gcc := &gcs.GuestConnectionConfig{
			Conn:     conn,
			Log:      logrus.WithField(logfields.UVMID, vm.ID), // TODO: JTERRY75 - should this be log.G(ctx)
...
func (vm *VirtualMachineSpec) Stop(force bool) error {
	...
	// BEGIN if external gcs is used
	err = vm.shutdown(ctx)
	if err != nil {
		return err
	}
	// END if external gcs is used
...
func (vm *VirtualMachineSpec) shutdown(ctx context.Context) (err error) {
	return vm.gc.Shutdown(ctx, "00000000-0000-0000-0000-000000000000")
}
